...
def sistergift(request,data=None):
    ...
    if data==None:
        ...
        sistergifts=Product.objects.filter(category='S').filter(artistname=data)
    elif data=='below':
    ...
    if request.user.is_authenticated:
        totalitem=len(Cart.objects.filter(user=request.user))
    return render(request, 'sistergift.html',{'sistergifts':sistergift,'totalitem':totalitem})
...
def painting(request,data=None):
    ...
    if data==None:
        ...
        paintings=Product.objects.filter(category='P').filter(artistname=data)
    elif data=='below':
    ...
    if request.user.is_authenticated:
        totalitem=len(Cart.objects.filter(user=request.user))
    return render(request, 'painting.html',{'painting':paintings,'totalitem':totalitem})
...
def lovecard(request,data=None):
    ...
    if data==None:
        ...
        lovecards=Product.objects.filter(category='L').filter(artistname=data)
    elif data=='below':
...
def birthdaycard(request,data=None):
    ...
    if data==None:
        ...
        birthdaycards=Product.objects.filter(category='B').filter(artistname=data)
    elif data=='below':
